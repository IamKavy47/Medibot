...
with st.container():
    ...
    with chat_col:
        ...
        with st.container():
            ...
            # Chat input
            user_input = st.chat_input("Chat with Medibot...")
...
with st.sidebar:
    ...
    st.markdown("""
    🆘 **Emergency Helplines:**
    - National Crisis Helpline: +91-9820466726
    ...
    with col1:
        if st.button("Export History as TXT"):
            # Convert chat history to a string format suitable for .txt
            chat_history_txt = ""
            for chat in st.session_state.chat_history:
                chat_history_txt += f"Date: {chat['date']}\n"
                chat_history_txt += f"Summary: {chat['summary']}\n"
                chat_history_txt += "Messages:\n"
                for message in chat['messages']:
                    chat_history_txt += f"  - {message['role'].capitalize()}: {message['content']}\n"
                chat_history_txt += "\n"  # Separate each chat
...
if user_input:
    ...
    try:
        ...
        context = """You are MediBot, a compassionate mental health support chatbot. 
